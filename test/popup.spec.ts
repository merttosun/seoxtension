...
describe("popup works like a charm", () => {
  ...
  let mockTTFB: HTMLSpanElement,
    mockFCP: HTMLSpanElement,
    mockDOMLoadTime: HTMLSpanElement,
    mockWindowLoadTime: HTMLSpanElement;
  ...
  beforeEach(() => {
    ...
    mockSetPerformanceMetrics = jest.fn(() =>
  ...
  test("setPerformanceMetrics should set performance properties to elements", () => {
    ...
    const window_load_time = document.getElementById(
      "window-load-time"
    ) as HTMLSpanElement;
    const dom_load_time = document.getElementById(
      "dom-load-time"
    ) as HTMLSpanElement;
    ...
    expect(window_load_time.innerText).toBe(
      mocks.performanceMetrics.windowLoadTime
    );
  });
