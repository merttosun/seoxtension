...
setInterval(async () => {
  ...
  if (!metaTagsFetched) {
    chrome.runtime.sendMessage({ msg: CHROME_MESSAGE.META }, function (response: META_DATA) {
  ...
  if (!imagesFetched) {
    chrome.runtime.sendMessage({ msg: CHROME_MESSAGE.IMAGE }, function (response: IMAGE_DATA) {
  ...
  if (!anchorsCountFetched) {
    chrome.runtime.sendMessage({ msg: CHROME_MESSAGE.ANCHOR }, function (response: LINK_DATA) {
      if (response?.length) {
  ...
  if (!ldJsonsFetched) {
    chrome.runtime.sendMessage({ msg: CHROME_MESSAGE.LD_JSON }, function (response: string[]) {
      if (response) {
        ldJsonsFetched = true
      }
